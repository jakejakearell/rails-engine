...
describe "Rail Engine API-Revenue" do
  ...
  describe 'Happy Paths' do
    it "can get the total revenue for a given merchant" do
      ...
      expect(item[:data][:id].to_i).to eq(@merchant_1.id)
      expect(item[:data]).to have_key(:type)
      expect(item[:data][:type]).to eq("merchant_revenue")
      expect(item[:data]).to have_key(:type)
      expect(item[:data][:type]).to eq("merchant_revenue")

      expect(item[:data]).to have_key(:attributes)
      expect(item[:data][:attributes]).to be_a(Hash)
      expect(item[:data][:attributes]).to have_key(:revenue)
      expect(item[:data][:attributes][:revenue]).to eq(100)
    end
  end
end
